...
**D8 — the pack list is derived, not restated.** `.vale.ini`'s `Packages =` is the source of truth, and three places enumerate it by hand: the rendered `ci.yml`'s cache `path:`, the rendered pre-commit `vale-sync` hook's `for p in`, and `.vale.ini`'s own `BasedOnStyles`. None failed loudly on drift, and a `BasedOnStyles` entry `Packages` does not fetch makes vale exit `E100 [loadStyles]` — which the gate would report as "rendered prose must be clean", the packs-versus-prose misdiagnosis its `vale sync` guard exists to prevent. A `vale pack-list lockstep` step derives the set and asserts all three; the gate consumes it rather than becoming a fourth. `actions/cache` takes no shell variable, so the downstream cache list stays literal and is asserted rather than generated.
...
**D10 — the gate caches the style packs, and the cache is load-bearing.** An earlier revision mirrored the rendered `ci.yml`'s cache step and it did nothing: `vale sync` never consults a cache, it wipes and re-creates every directory in `Packages =` on every invocation (verified with canary files — both were gone after a second sync). A restored cache is discarded before it is read unless the sync itself is skipped. The gate therefore syncs only when a pack directory is missing, the same decision the `vale-sync` pre-commit hook already makes, and the cache carries **no `restore-keys`**: with a guarded sync, a partial-key restore would hand the gate a stale pack set and then skip the sync that would have corrected it — the #159 clobber in a new place. An exact key only, so a `.vale.ini.jinja` change misses and re-resolves.

This accepts a trade. `Packages =` lists the registry packs without an `@version`, which `.vale.ini.jinja` calls intentional ("keeps style rules current"). On a cache hit the gate lints against the packs as they were when the key was written rather than the latest release, so a rule refresh now happens when `.vale.ini.jinja` changes rather than continuously. That is the point: it is what stops an upstream rule landing mid-PR and failing the gate on prose nobody touched.

The rendered `ci.yml` **cannot** get the same treatment. `vale-cli/vale-action` runs `vale sync` unconditionally at `src/input.ts:67` and `action.yml` exposes no input to skip it (both checked at the pinned SHA `85f9f7f`), so a downstream's cache step is inert and stays inert while that action is used. Making it load-bearing would mean replacing the action with a direct invocation, giving up reviewdog annotations, `filter_mode: added` and `fail_on_error`. Out of scope here; the step keeps a note saying so.
