...
**D10 — the gate does not cache the style packs.** An earlier revision mirrored the rendered `ci.yml`'s cache step. It cannot work: `vale sync` wipes and re-creates every directory in `Packages =` rather than consulting what is on disk, so a restored cache is discarded before it is read (verified with canary files). The step was removed rather than kept as decoration. The gate therefore depends on the pinned `ai-tells` release URL staying live, which `.vale.ini.jinja` already warns about; making a cache load-bearing would mean guarding the sync on directory existence, trading away the deliberate "latest registry release on each sync" behaviour — a fleet-wide call, and the rendered `ci.yml`'s equivalent step is inert for the same reason.
