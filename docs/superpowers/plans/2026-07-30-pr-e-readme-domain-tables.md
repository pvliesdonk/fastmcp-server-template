...
**E2 — CORE reproduces today's content.** The `readme` tag on `kv_store_url` (core) and the two observability vars (config-presentation) yields exactly the current three rows. Assert the generated CORE table equals today's Configuration table content (order + rows), so this half is provably non-breaking.
