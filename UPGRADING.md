...
- **If you ran `ruff format` over `tests/test_health.py`** after an earlier
  update (any `project_name` of 18 characters or more), `copier update` may
  report a conflict around the two `/<project_name>/health` asserts. Keep
  the template side (the `health = "/<project_name>/health"` variable plus a
  one-line assert) and drop your wrapped form. Then check that
  `uv run ruff format --check tests/test_health.py` passes.
